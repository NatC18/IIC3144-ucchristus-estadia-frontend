...
import { getEstadoGestionColor, getHospitalizacionColor, getServicioColor } from '@/lib/transformations'
...
describe('getServicioColor', () => {
  it('returns blue classes when servicio is TRASLADO', () => {
    expect(getServicioColor('TRASLADO')).toBe(
      'bg-blue-100 text-blue-800 rounded-full whitespace-nowrap'
    )
  })

  it('returns green classes when servicio is EGRESO', () => {
    expect(getServicioColor('EGRESO')).toBe(
      'bg-green-100 text-green-800 rounded-full whitespace-nowrap'
    )
  })

  it('returns gray classes when servicio is INGRESO', () => {
    expect(getServicioColor('INGRESO')).toBe(
      'bg-gray-100 text-gray-800 rounded-full whitespace-nowrap'
    )
  })
})